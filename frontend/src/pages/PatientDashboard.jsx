      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-4">
              {mockAppointments.map((appt) => (
                <div key={appt.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
                  ...
                  <span className="mt-2 sm:mt-0 px-3 py-1 rounded-full text-xs font-semibold uppercase bg-green-100 text-green-700">
                    {appt.status}
                  </span>
                </div>
              ))}
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm flex flex-col items-center text-center">
            <div className="h-24 w-24 rounded-full mb-4 bg-blue-100 flex items-center justify-center text-blue-600">
              <User className="h-10 w-10" />
            </div>
            <h2 className="text-xl font-bold">John Doe</h2>
            <p className="text-sm text-gray-500 mb-4">Patient ID: #PAT-8832</p>
            <button onClick={() => navigate('/profile')} className="w-full py-2 bg-gray-100 dark:bg-gray-800 font-semibold rounded-xl text-sm">Manage Account</button>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <button onClick={() => navigate('/records')} className="flex items-center gap-3 p-3.5 rounded-xl bg-gray-50 hover:bg-blue-50 dark:bg-gray-800/50 dark:hover:bg-blue-900/20 text-left transition-all">
                <div className="bg-emerald-600 text-white p-2 rounded-lg"><FileText className="h-4 w-4" /></div>
                <div>
                  <div className="font-semibold text-sm">Medical Records</div>
                  <div className="text-xs text-gray-400">View prescriptions</div>
                </div>
              </button>

              <button onClick={() => setIsChatOpen(true)} className="flex items-center gap-3 p-3.5 rounded-xl bg-gray-50 hover:bg-blue-50 dark:bg-gray-800/50 dark:hover:bg-blue-900/20 text-left transition-all">
                <div className="bg-cyan-500 text-white p-2 rounded-lg"><MessageSquare className="h-4 w-4" /></div>
                <div>
                  <div className="font-semibold text-sm">Telehealth Chat</div>
                  <div className="text-xs text-gray-400">Talk to your Care Team</div>
                </div>
              </button>

              <button onClick={() => navigate('/connect')} className="flex items-center gap-3 p-3.5 rounded-xl bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 text-left transition-all">
                <div className="bg-blue-600 text-white p-2 rounded-lg"><QrCode className="h-4 w-4" /></div>
                <div>
                  <div className="font-bold text-sm text-blue-900 dark:text-blue-300">Link with Doctor</div>
                  <div className="text-xs text-blue-700/70">Enter Clinic ID</div>
                </div>
              </button>
