        ...
        <div className="bg-white dark:bg-gray-900 max-w-md w-full rounded-3xl shadow-sm border border-gray-200 dark:border-gray-800 p-8 text-center">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => navigate('/pharmacy')}
              className="flex-1 border border-gray-200 dark:border-gray-700 font-bold py-3 rounded-xl text-gray-600 dark:text-gray-300">
              Keep Shopping
            </button>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/pharmacy')} className="bg-indigo-600 text-white font-bold px-6 py-3 rounded-xl">
            Browse Pharmacy
          </button>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Delivery */}
            <section className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6">
              <h2 className="font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                <Truck className="h-5 w-5 text-indigo-600" /> Delivery Address
              </h2>
              <textarea value={address} onChange={(e) => setAddress(e.target.value)} rows={3}
                placeholder="Full delivery address, including postal code…"
                className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
            </section>
            ...
            <section className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                {['card', 'paypal', 'applepay'].map((m) => (
                  <button key={m} onClick={() => setPayment(m)}
                    className={`py-3 rounded-xl text-xs font-bold border capitalize transition-colors ${
                      payment === m ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300'
                    }`}>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/40 rounded-xl p-3 flex items-start gap-2 text-xs text-blue-700 dark:text-blue-300">
                ...
                Payment is simulated in this demo. For production, integrate Stripe so card data never touches your server.
              </div>
            </section>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 sticky top-6">
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto mb-4">
                {cart.map((i) => (
                  <div key={i.medicine_id} className="flex items-center gap-3">
                    <div className="h-10 w-10 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded-lg flex items-center justify-center shrink-0">
              ...
              <div className="border-t border-gray-100 dark:border-gray-800 pt-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between text-gray-500">
                  <span>Shipping</span>
                  <span>{shipping === 0 ? <span className="text-emerald-600 font-bold">FREE</span> : money(shipping)}</span>
                </div>
              ...
              <button onClick={placeOrder} disabled={placing}
                className="w-full mt-5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/30">
