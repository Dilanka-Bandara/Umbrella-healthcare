...
const Navbar = () => {
  ...
  // Helper function to route users to their specific dashboard
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0">
            ...
            <span className="font-black text-xl text-gray-900 dark:text-white hidden sm:block">Umbrella Medical</span>
          </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {/* 🛒 Smart Cart Icon (Only show for Patients) */}
            {user?.role === 'patient' && (
              <button 
            ...
            {/* 🚨 STAFF LINKS (Admin & Pharmacist) */}
            {(user?.role === 'admin' || user?.role === 'pharmacist') && (
              <div className="hidden sm:flex items-center gap-4 mr-2">
                <button 
                  onClick={() => navigate('/fulfillment')} 
                  className="text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-bold text-sm transition-colors"
                >
                  Fulfillment
                </button>
              </div>
            ...
            {user ? (
              <>
                <span className="hidden lg:block text-sm font-semibold text-gray-600 dark:text-gray-300 ml-2 truncate max-w-[120px]">
                ...
                <button 
                  ...
                  className="flex items-center gap-1.5 text-sm font-bold text-blue-600 hover:text-blue-700 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 px-4 py-2 rounded-xl transition-colors"
                >
                ...
                <button 
                  ...
                  className="flex items-center gap-1.5 text-sm font-bold text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 px-4 py-2 rounded-xl transition-colors"
                  title="Sign Out"
                >
              ...
              <button 
                ...
                className="text-sm font-bold px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-full transition-all shadow-md ml-2"
              >
                Sign In
              </button>
            )}
